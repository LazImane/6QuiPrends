    ...
    def evaluatepotentialscore(self , plays , table_state) :
        ...
        for player, card in plays:
            ...
            if not placed:
                ...
                self.sort_table(newtablestate)
            #print('score' + str(score) + 'new table state' + str(newtablestate))
...
# appel à minimax(self.hand , game.table , profondeur , +inf, -inf , TRUE)
    ...
    #def minimax(self, state , table_state , profondeur , maxplayer): 
        if profondeur == 0 :
            ...
            return possiblechoices[0][0].value
        if maxplayer: 
            value = float('inf')
        ...
        while True:
            ...
            try:
                ...
                if(len(self.hand) <= 2) : 
                    ...
                    carteChoisie = Card(self.minimax(self.hand  , self.gamestate ,[], 3 , float('inf') , float('-inf') , True))
